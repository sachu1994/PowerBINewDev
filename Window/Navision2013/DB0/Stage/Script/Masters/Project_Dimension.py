...
for dbe in config["DbEntities"]:
    if dbe['ActiveInactive']=='true':
        ...
        try:
            cur = conn.cursor()
            cur.execute('DROP TABLE IF EXISTS Masters.PROJECT_Dimension;')  
            conn.commit()
            conn.close()
        ...
        try:
            ...
            for i in range(NoofRows):    
                ...
                Path = Abs_Path+"/"+DBName+"/"+EntityName+"\\Stage2\\ParquetData\\Master\PROJECT_Dimension"
            ...
            try:
                ...
                IDEorBatch = "IDLE"
            os.system("spark-submit "+Kockpit_Path+"\Email.py 1 PROJECT_Dimension '"+CompanyName+"' "+DB0+" "+str(exc_traceback.tb_lineno)+"")    
            log_dict = logger.getErrorLoggedRecord('Masters.PROJECT_Dimension', DB0," ", str(ex), exc_traceback.tb_lineno, IDEorBatch)
            log_df = spark.createDataFrame(log_dict, logger.getSchema())
            log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
print('masters_PROJECT_Dimension completed: ' + str((dt.datetime.now()-st).total_seconds()))     
