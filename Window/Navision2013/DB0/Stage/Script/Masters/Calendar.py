...
for dbe in config["DbEntities"]:
    if dbe['ActiveInactive']=='true':
        ...
        try:
            cur = conn.cursor()
            cur.execute('DROP TABLE IF EXISTS Masters.Calendar;')  
            conn.commit()
            conn.close()
        ...
        try:
            ...
            for i in range(NoofRows):   
                ...
                Path = Abs_Path+"/"+DBName+"/"+EntityName+"\\Stage2\\ParquetData\\Master\Calendar"
            ...
            try:
                ...
                IDEorBatch = "IDLE"
            os.system("spark-submit "+Kockpit_Path+"\Email.py 1 Calendar '"+CompanyName+"' "+DB0+" "+str(exc_traceback.tb_lineno)+"")    
